...
└── tests/
    ├── incremental_regression.py   # 58 条断言：九类哈希的增量触发口径 + 字段结构提取准确性
...
断言覆盖：全量 / 增量模式切换、行号整体位移与辅助文件改动**不得**触发、新增路由 / 建表语句加列 / 引入 MQ **必须**触发、DDL 加列联动接口页、请求字段名·类型·必填准确性（含多变量 `not` 守卫、`not in` 枚举守卫与纯内容分支的区分）、响应结构回溯（dict 字面量 / 局部变量 / `helper()` / `append` / `SELECT` 兜底 / 二进制下载，含**右值带行尾注释**与**字典字面量夹行内注释**两种写法）、DDL 列抽取保真（`key` 列、字符串拼接建表、同名小表不得覆盖核心表、动态补列清单）。
